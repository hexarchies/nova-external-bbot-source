...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float spacingx = style.ItemInnerSpacing.x - 5; // Расстояние между квадратом и текстом
    const float spacingy = style.ItemInnerSpacing.y - 20; // Расстояние между квадратом и текстом
    const ImVec2 pos = window->DC.CursorPos + ImVec2(square_sz + spacingx, square_sz + spacingy); // Сдвигаем позицию на square_sz + spacing вправо
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() + 25, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
